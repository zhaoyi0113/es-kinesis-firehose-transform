...
const { testData, testJsonData } = require('./sample/logs/kinesisLog');
...
describe('test log', () => {
  ...
  it('should be able to ingest json log', async () => {
    await axios.post(logEndpoint, JSON.stringify(testJsonData[0]), {
      headers: { 'Content-Type': 'application/json' },
    });
    const index = getIndexName('logs');
    const docs = await esclient.search({
